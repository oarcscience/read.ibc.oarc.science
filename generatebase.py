...
for f in glob.glob("_data/sources getbible-net/*.txt"):
    ...
    ls="¤".join(map(str,list(df[["book","chapter"]].groupby("book",sort=False).agg(set)["chapter"].apply(len))))
    lscum="¤".join(map(str,[0]+list(df[["book","chapter"]].groupby("book",sort=False).agg(set)["chapter"].apply(len).cumsum())))
